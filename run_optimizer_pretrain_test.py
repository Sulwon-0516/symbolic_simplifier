...
def pretrain():
    base_r = 0
    for depth, max_epoch in zip([2, 3, 4], [10, 5, 5]): # zip([2,3,4], [1,0,0]):
            ...
            for idx in range(3):
                ...
                for tree, _ in pretrain_batcher.level(depth, *params):
                    ...
                    r, loss, equals = e2d.tree_beam_search(
                        ...
                        beam_size=20, num_res=20, 
                        train=params[0], 
                        baseline=base_r)
                    
                    if len(equals):
                        # when there is more than 1 equivalence...
                        # Add the result on the rules.
                        hits[idx] += 1
...
def pipeline_train(e2d, picker, log_prob, picker_reward, train):
    ...
    res = [d.grad for d in picker.parameters() if d.grad is not None and torch.any(torch.isnan(d.grad))]
    if len(res):
        return
    ...
    while n_iter < MAX_ITER:
        ...
        if train and False:
            ...
            for k, v in sub_LUT.items():
                if k in LUT and k not in SEE:
                    merged_lut[v] = LUT[k]
            # with changed encoder, do beam-search again
            e2d.tree_beam_search(
                [sub_tree], vocab,
                max_depth=min(3, sub_tree.depth()),
                beam_size=20,
                num_res=20, 
